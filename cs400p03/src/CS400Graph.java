...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      // copying and updating variables for the new Path
      ...
      this.dataSequence.add(extendBy.target.data);// adding the new node to the path
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    // Finds the according vertices based on the arg
    Vertex sV = vertices.get(start);
    Vertex eV = vertices.get(end);
    // If either vertex variable is returned null (vertex that has no matching data
    // var) throw a NSE exception
    if (sV == null || eV == null) {
    ...
    frontier.add(new Path(sV));

    ArrayList<Vertex> visited = new ArrayList<Vertex>();

    // loop that runs until the frontier is empty, if frontier is empty a NSE
    // exception is thrown
    while (!frontier.isEmpty()) {

      closest = frontier.poll(); // pops the frontier for the closest path
      visited.add(closest.end); // adds the last vertex of the new path to visited
      temp = closest.end;
      // checks if the shortest path is found
      if (closest.end.data.equals(end)) {
        return closest;
      }
      // adds new paths to the frontier based on every non visited node of the
      // furthest edge
      for (Edge e : temp.edgesLeaving) {
        if (!visited.contains(e.target)) {
          frontier.add(new Path(closest, e));
        }
      }
    }
    throw new NoSuchElementException();
